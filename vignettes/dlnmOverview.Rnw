...
Exposure-lag-response associations are modelled through an extended version of DLNM. \citet{gasparrini2013statmed} provides the methodological development and the definitions above.
...
The lag or cross-basis matrix can be included in the design matrix of a regression model in order to estimate the related parameters. The algebraic definitions of the models above are provided in \citet{gasparrini2010statmed} and \citet{gasparrini2013statmed}.
...
Again, the algebraic definitions of the models above are provided in \citet{gasparrini2010statmed} and \citet{gasparrini2013statmed}.
...
This is the main function in the package \Rpkg{dlnm}. It internally calls \Rfun{onebasis} to generate the basis matrices for exposure-response and lag-response relationships, and combines them through a special tensor product in order to create the cross-basis, which specifies the exposure-lag-response dependency simultaneously in the two dimensions. See \citet[Sections 2.1--2.2]{gasparrini2013statmed} and \citet[Sections 4.1--4.2]{gasparrini2010statmed} for details.
...
The package \Rpkg{dlnm} also contains a set of functions which are called internally by the other functions illustrated above, in particular \Rfun{onebasis}, \Rfun{crossbasis} and \Rfun{crosspred}. Following the usual \R{} style, these functions have names starting with a dot. Some of this functions are documented, and help pages are opened with the usual call (results not shown):
