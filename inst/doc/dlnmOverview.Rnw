...
The function calls \Rfun{onebasis} and passes the arguments in \Rarg{argvar} and \Rarg{arglag} to build the basis for predictor and lags, respectively. In this case, we assume that the effect of \PM{} is linear (\Rarg{type="lin"}), while we model the relationship with temperature through a natural cubic spline with 5 degrees of freedom (\Rarg{type="ns"}, chosen by default). In this space, the internal knots (if not provided) are placed by default at equally spaced quantiles, while the boundary knots are located at the range of the observed values, so we need to specify only \Rarg{df}. We did not center \PM{}, in order to compute the predicted effects versus a reference value of 0 \microg{} (the same results could be obtained setting \Rarg{cen=0}). The reference value for temperature is set to 21\Ctemp{}.
