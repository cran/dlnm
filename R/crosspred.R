...
if(is.null(at)) {
  ...
  if(dim(at)[2]!=length(predlag)) {
    stop("matrix in 'at' must have ncol=seq(lag[1],lag[2],bylag)")
  }
...
colnames(matfit) <- colnames(matse) <- outer("lag",.seq(attr$lag,bylag),
  paste,sep="")
